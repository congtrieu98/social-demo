...
export const LoginFormModal: React.FunctionComponent<
  PropsWithChildren
> = () => {
  const [signUpAccount, setSignUpAccount] = useState(false);
  const [forgotPassword, setForgotPassword] = useState<Boolean>(false);

  console.log("forgotPassworddđ:", forgotPassword)
  return (
    <>
      <div className="flex flex-col transform rounded-2xl pt-8 items-center justify-center gap-5 transition-all">
        <button className="absolute -top-12 flex p-2 flex-col items-start gap-1 self-stretch rounded-full w-full">
          <div className="flex gap-2 w-full">
            ...
            <div className="flex-1 text-white text-center text-[15px] font-semibold leading-6">
              {forgotPassword ? 'Quên mật khẩu' : signUpAccount ? 'Đăng ký' : 'Đăng nhập'}
            </div>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="15"
        ...
        <div className="flex flex-col gap-5">
          <div className="text-left flex flex-col gap-2.5">
            <div className="text-[23px] font-semibold leading-7 text-slate-700 ">
              {forgotPassword ? 'Quên mật khẩu rồi 🤧!' : signUpAccount ? 'Vẫn chưa có tài khoản lun🙄!' : 'Chào mừng đến SuZu nà!'}
            </div>
            {
              forgotPassword ?
          ...
          <div className="flex flex-col  gap-2.5">
            ...
            <div className="w-full">
              {
                forgotPassword ? <ForgotPass /> : signUpAccount ? <SignupForm /> : <LoginForm setForgotPassword={setForgotPassword} />
              }
            </div>
          </div>
        </div>
        ...
        <div className="flex px-4 pt-3 pb-6 justify-center items-center border-t border-slate-100 w-full">
          <div className="flex text-slate-500 text-center">
            {signUpAccount || forgotPassword ? 'Bạn đã có tài khoản 😉?' : 'Bạn vẫn chưa có tài khoản 🙄?'} {" "}
            <button
              onClick={() => setSignUpAccount(!signUpAccount)}
              className="text-slate-900 text-[15px] font-semibold leading-6 ml-2"
            >
              {" "}
              {signUpAccount || forgotPassword ? 'Đăng nhập nào' : 'Đăng ký ngay'}
            </button>
          </div>
        </div>
      </div>
